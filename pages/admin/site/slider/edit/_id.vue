<template>
  <div>
    <h1 class="mb-17">{{ isNew ? '이미지 슬라이드 등록' : '이미지 수정' }}</h1>
    ...
    <div class="info-table">
      ...
      <div class="row">
        <label>부제목</label>
        <div>
          <SInput v-model="detailData.subtitle" w-size="xxx-large" maxlength="100" />
        </div>
      </div>
      <div class="row">
        <label>일정</label>
        <div>
          <SInput v-model="detailData.schedule" w-size="xxx-large" />
        </div>
      </div>
      <div class="row">
        <label>노출여부</label>
      ...
      <div class="row">
        ...
        <div class="col">
          ...
          <p class="bt-2r">가로 1920px 세로 800px 10MB 이하의 PNG, JPG 파일</p>
        </div>
      </div>
      <div class="row">
        <label>PC 이미지 대체 텍스트</label>
        <div>
          <STextarea v-model="detailData.pcImageAlt" maxlength="400" />
        </div>
      </div>
      <div class="row">
        <label>Mobile 이미지<b>*</b></label>
        <div class="col">
          <div>
            <SInput
          ...
          <p class="bt-2r">가로 720px 세로 1256px 10MB 이하의 JPG, PNG 파일</p>
        </div>
      </div>
      <div class="row">
        <label>Mobile 이미지 대체 텍스트</label>
        <div>
          <STextarea v-model="detailData.mobileImageAlt" maxlength="400" />
        </div>
      </div>
      <div class="row">
        <label>버튼 사용 여부</label>
        <div>
          <SToggle v-model="detailData.isUseButton">{{ detailData.isUseButton ? '사용' : '미사용' }}</SToggle>
        </div>
      </div>
      <div class="row">
        <label>버튼 링크</label>
...
import { SLIDES_DETAIL } from '~/assets/js/types';
import { createFormElement, fileValid } from '~/assets/js/commons';
import { sliderPreview } from '~/assets/js/previewConverter';
export default {
  name: 'SliderEditPage',
  components: { STextarea, SDialogModal, SCheckbox, SInput, SToggle, SDatepicker, SButton },
  layout: 'admin/default',
  async asyncData({ params, $axios, redirect, $dayjs }) {
    const { id } = params;
    const isNew = id == null;
    const detailData = isNew
      ? cloneDeep(SLIDES_DETAIL)
      : await $axios.$get(`/admin/slides/${id}`).catch(() => {
          redirect('/admin/site/slides');
        });
    if (isNew) {
  ...
  methods: {
    async onRegister() {
      ...
      if (this.isValidate()) {
        ...
        if (this.isNew) {
          await this.$axios.$post(`/admin/slides`, detailData);
        } else {
          await this.$axios.$put(`/admin/slides/${this.id}`, detailData);
        }
    ...
    isValidate() {
      ...
      if (!startDate || startDate === 'Invalid Date' || !endDate || endDate === 'Invalid Date') {
        this.feedback.date.text = '진행기간을 선택해 주세요.';
    ...
    onDeletePost() {
      this.isConfirmPending = true;
      this.$axios.$delete(`/admin/slides/${this.id}`);
      this.modal.isRemoveConfirm = true;
      this.isConfirmPending = false;
    },
    showPreview() {
      const convertData = { previewData: JSON.stringify(sliderPreview(this.detailData)) };
      const $formEl = createFormElement('/', convertData, true);
