<template>
  <div>
    <div class="top">
      <STitle class="mb-16 mr-16" emoji="pencil">전시 {{ isNew ? '등록' : '수정' }}</STitle>
      ...
      <div class="left">
        <div v-scroll-spy="{ allowNoActive: true }">
          <section data-index="0" class="mb-36">
            ...
            <div class="info-table">
              <div class="row">
                <label>전시기간<b class="must">*</b></label>
                <div class="has-feedback">
                  <div>
                    <SDatepicker
    ...
    <SDialogModal :is-show="modal.isCancel" @close="modal.isCancel = false">
      ...
      <template #modal-btn2>
        <SButton button-type="primary" @click="$router.push('/admin/common/exhibition')">확인</SButton>
      </template>
    </SDialogModal>
    ...
    <SDialogModal :is-show="modal.isSave" @close="modal.isSave = false">
      ...
      <template #modal-btn1>
        <SButton button-type="primary" @click="$router.push(`/admin/common/exhibition/${id}`)">확인</SButton>
      </template>
    </SDialogModal>
    ...
    <SDialogModal :is-show="modal.isNewSave" @close="$router.push(`/admin/common/exhibition`)">
...
import RoundModal from '~/components/admin/modal/RoundModal';
...
export default {
  ...
  async asyncData({ $axios, params, $dayjs }) {
    ...
    const detailData = isNew
      ...
      : await $axios.$get(`/admin/exhibitions/${id}`).then((detailData) => ({
          ...detailData,
  ...
  methods: {
    ...
    async saveData() {
      ...
      if (this.isNew) {
        await this.$axios
          .$post('/admin/exhibitions', detailData)
          .then(() => {
        ...
        await this.$axios
          .$put(`/admin/exhibitions/${this.id}`, detailData)
          .then(() => {
