  ...
  <div>
    <div class="top-banner mb-28">
      ...
      <div class="bottom">
        ...
        <div class="btn-area">
          <SLink to="/admin/common/exhibition" class="mr-8" button-type="transport-b">리스트</SLink>
          <SButton class="mr-8" button-type="transport-b" @click="showPreview">미리보기</SButton>
          <SLink :to="`/admin/common/exhibition/edit/${id}`" button-type="primary">수정</SLink>
        </div>
      </div>
    </div>
    ...
    <div class="edit-area">
      <div class="left">
        <div v-scroll-spy="{ allowNoActive: true }">
          ...
          <section data-index="1" class="statistics mb-36">
            <h3 class="mb-24">
              전시 통계
              <SLink :to="`/admin/common/exhibition/${id}/chart`" button-type="transport-b">더보기</SLink>
            </h3>
            <SStatisticsCard
          ...
          <section data-index="2" class="note mb-36">
            ...
            <div class="info-table">
              ...
              <div class="row">
                ...
                <div>{{ isHanaDiscount ? `/hana/hana-pay-entry?id=${detailData.id}&type=exhibition` : '-' }}</div>
              </div>
              <div class="row">
      ...
      <div class="right ml-28">
        <nav v-scroll-spy-active v-scroll-spy-link class="side-menu">
          ...
          <a class="bt-1b">전시 통계</a>
          <a class="bt-1b">비고</a>
          <a class="bt-1b">운영시간 / 입장권</a>
          <a class="bt-1b">전시 콘텐츠</a>
...
export default {
  ...
  async asyncData({ $axios, params, $dayjs }) {
    const { id } = params;
    const detailData = await $axios.$get(`/admin/exhibitions/${id}`);
  ...
  fetch() {
    ...
    this.$axios
      .$get(`/admin/tickets/statistics/dashboard?kind=EXHIBITION&id=${this.id}`)
      .then((statisticsList) => {
  ...
  methods: {
    ...
    async updateNote() {
      await this.$axios.$put(`/admin/exhibitions/${this.id}/note`, { data: this.detailData.note });
    },
    ...
    showPreview() {
      ...
      const $formEl = createFormElement('/exhibition/preview', convertData, true);
