<template>
  <div>
    <div class="top">
      <NuxtLink :to="`/admin/common/exhibition/${id}`" class="prev-page">
        <i class="ic-arrow-left" />
      </NuxtLink>
      <STitle emoji="chart">프로그램 통계</STitle>
      <div>
    ...
    <div class="two-grid mb-36">
      <div class="summary">
        ...
        <div v-if="summaryData" class="summary-wrap">
          <div class="summary-top mb-24">
            <div class="left">
              입장<b class="nt-1">{{ getThreeCommaNum(summaryData.visitNumber) }}</b>
              <span>명</span>
              <SDivLine />
              입장예정<b class="nt-1">{{ getThreeCommaNum(summaryData.bookingNumber) }}</b>
              <span>명</span>
            </div>
            <div class="right">
      ...
      <ChartByPurchaseOptions :ex-start-date="exStartDate" :ex-end-date="exEndDate" kind="PROGRAM" :kind-id="id" />
    </div>
    <ChartByPeriod
...
export default {
  ...
  async fetch() {
    this.summaryData = await this.$axios.$get('/admin/statistics/summary', {
      params: { startDate: this.exStartDate, endDate: this.exEndDate, kind: 'PROGRAM', kindId: this.id }
    });
  },
  computed: {
    exStartDate() {
