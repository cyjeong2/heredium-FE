<template>
  <div>
    <div class="top">
      <STitle class="mb-16 mr-16" emoji="pencil">프로그램 {{ isNew ? '등록' : '수정' }}</STitle>
    ...
    <div class="edit-area">
      <div class="left">
        <div v-scroll-spy="{ allowNoActive: true }">
          <section data-index="0" class="mb-36">
            ...
            <div class="info-table">
              <div class="row">
                <label>프로그램기간<b class="must">*</b></label>
                <div class="has-feedback">
                  <div>
                    <SDatepicker
            ...
            <div class="round-inner">
              ...
              <div class="round-wrap">
                <div class="round-menu-top mb-16">
                  <p class="h4">
                    {{ selectedRoundDate ? $dayjs(selectedRoundDate).format('YYYY년 MM월') : '-' }}
                    <span v-if="currentMonthRoundData.length > 0" class="ml-8 bt-2r"
                      >총 {{ currentMonthRoundData.length }}회차</span
                    >
                  </p>
                  <SButton button-type="transport-b" @click="openEditModal(true)">등록</SButton>
                </div>
                <div class="round-list">
                  <template v-if="currentMonthRoundData.length > 0">
                    <div v-if="currentMonthRoundData.length === 0" class="round-item-empty">리스트가 없습니다.</div>
                    <div v-for="(item, index) in currentMonthRoundData" :key="index" class="round-item">
                      <p class="count">{{ index + 1 }}</p>
                      <div class="info">
                        <div class="top">
                          <div class="nt-3">{{ $dayjs(item.date).format('YY.MM.DD') }}</div>
                          <div class="item-edit">
                            <button type="button">
                              <i class="ic-edit" @click="modifyRound(item, index)" />
                            </button>
                            <button
                              v-if="item.id === null || getIsShowDelete(item.date)"
                              type="button"
                              @click="checkDeleteRound(item)"
                            >
                              <i class="ic-trash" />
                            </button>
                          </div>
          ...
          <section data-index="4" class="editor mb-36">
            <h3 class="mb-16">프로그램 콘텐츠<b class="must">*</b></h3>
              ...
              <div v-for="(group, groupIndex) in detailData.writers" :key="groupIndex" class="artist-group-item">
                <div class="artist-group-title">
                  ...
                  <div class="title">
                    ...
                    <button type="button" @click="deleteArtistGroup(groupIndex)"><i class="ic-trash" /></button>
                  </div>
                ...
                <div v-if="group.isShowDetail" class="artist-wrap" :class="{ 'is-error': group.isNoArtist }">
                  ...
                  <Draggable v-model="group.infos" :group="{ name: 'artist' }" class="artist-list" v-bind="dragOptions">
                    <div v-for="(artist, artistIndex) in group.infos" :key="artist.id" class="artist-item">
                      ...
                      <div>
                        <span class="bt-1m">{{ artist.name || '이름 없음' }}</span>
                        <div class="btn-area">
      ...
      <div class="right ml-28">
        <nav v-scroll-spy-active v-scroll-spy-link class="side-menu">
          ...
          <a class="bt-1b">입장권 정보</a>
          <a class="bt-1b">프로그램 콘텐츠</a>
    ...
    <ProgramEditModal
      ...
      :modify-round-data="modifyRoundData"
      :is-new="isNewRound"
    ...
    <ArtistModifyModal
      ...
      @save="saveArtist"
    />
    <SDialogModal :is-show="modal.isDeleteRound" @close="modal.isDeleteRound = false">
      <template #content>리스트를 삭제하시겠습니까?</template>
      <template #modal-btn1>
        <SButton @click="modal.isDeleteRound = false">취소</SButton>
      </template>
      <template #modal-btn2>
        <SButton button-type="primary" @click="deleteRound">확인</SButton>
      </template>
    </SDialogModal>
    ...
    <SDialogModal :is-show="modal.isCancel" @close="modal.isCancel = false">
      ...
      <template #modal-btn2>
        <SButton button-type="primary" @click="$router.push('/admin/common/program')">확인</SButton>
      </template>
    </SDialogModal>
    ...
    <SDialogModal :is-show="modal.isSave" @close="modal.isSave = false">
      ...
      <template #modal-btn1>
        <SButton button-type="primary" @click="$router.push(`/admin/common/program/${id}`)">확인</SButton>
      </template>
    </SDialogModal>
    ...
    <SDialogModal :is-show="modal.isNewSave" @close="$router.push(`/admin/common/program`)">
...
import ArtistModifyModal from '~/components/admin/modal/ArtistModifyModal';
...
import ProgramEditModal from '~/components/admin/modal/ProgramEditModal';
...
export default {
  ...
  async asyncData({ $axios, params, $dayjs }) {
    ...
    const detailData = isNew
      ...
      : await $axios.$get(`/admin/programs/${id}`).then((detailData) => ({
          ...detailData,
  ...
  computed: {
    ...
    currentMonthRoundData() {
      const selectedYearMonth = this.$dayjs(this.selectedRoundDate).format('YYYY-MM');
      let targetRounds = [];

      Object.entries(this.computedRoundData).forEach(([key, value]) => {
        const dataDate = this.$dayjs(key).format('YYYY-MM');

        if (selectedYearMonth === dataDate) {
          targetRounds = [...targetRounds, ...value.rounds];
        }
      });
  ...
  methods: {
    getIsShowDelete(itemDate) {
      const DISABLED_ADD_DAY = 90;
      const programState = this.detailData.state;
      let isShowDelete = true;

      if (programState !== 'SCHEDULE') {
        const targetDate = this.$dayjs(itemDate);
        let disabledDay = '';

        if (programState === 'BOOKING') {
          disabledDay = this.$dayjs(this.detailData.startDate).add(DISABLED_ADD_DAY, 'd');
        } else if (programState === 'PROGRESS' || programState === 'TERMINATION') {
          disabledDay = this.$dayjs().add(DISABLED_ADD_DAY, 'd');
        }
        isShowDelete = targetDate.isSameOrAfter(disabledDay, 'day');
      }
    ...
    openEditModal(isNew) {
      this.isNewRound = !!isNew;
      this.modal.isRound = true;
    },
    setRound({ roundData, modifyIndex }) {
      const rounds = [...this.detailData.rounds];
      ...
      if (modifyIndex == null) {
        ...
        rounds[modifyIndex] = {
          ...
          limitNumber: roundData.limitNumber
        };
      }
      this.detailData.rounds = rounds;
      this.modal.isRound = false;
    },
    modifyRound(item, index) {
      this.modifyRoundData = { ...item, index };
      this.openEditModal(false);
    },
    checkDeleteRound(item) {
      this.deleteRoundId = item.id;
      this.modal.isDeleteRound = true;
    },
    deleteRound() {
      const deleteTargetId = this.deleteRoundId;
      const targetIndex = this.detailData.rounds.findIndex((item) => item.id === deleteTargetId);

      if (targetIndex != null) {
        this.detailData.rounds.splice(targetIndex, 1);
      }

      this.deleteRoundId = null;
      this.modal.isDeleteRound = false;
    },
    ...
    async saveData() {
      ...
      const detailData = {
        ...
        prices: this.detailData.prices.map((price) => ({ ...price, isError: null }))
      };

      if (this.isNew) {
        await this.$axios
          .$post('/admin/programs', detailData)
          .then(() => {
        ...
        await this.$axios
          .$put(`/admin/programs/${this.id}`, detailData)
          .then(() => {
    ...
    showPreview() {
      ...
      const $formEl = createFormElement('/program/preview', convertData, true);
