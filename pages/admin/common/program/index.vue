<template>
  <div>
    <STitle emoji="camera">프로그램</STitle>
    ...
    <div class="top-menus mb-16">
      <SLink to="/admin/common/program/edit" class="mr-28">등록</SLink>
      ...
      <template #data="{ data }">
        <table class="admin-table">
          ...
          <tbody>
            ...
            <tr v-for="(item, index) in data" :key="item.id" @click="$router.push(`/admin/common/program/${item.id}`)">
              <td>
...
export default {
  ...
  async fetch() {
    ...
    this.tableData = await this.$axios.$get('/admin/programs', {
      params: {
        ...this.queryOptions,
  ...
  methods: {
    ...
    getStateLabel(state) {
      return EXHIBITION_STATE_TYPE[state] || '';
    }
  }
};
</script>
