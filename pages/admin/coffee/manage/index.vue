<template>
  <div>
    ...
    <div class="top-menus mb-16">
      <SLink to="/admin/coffee/manage/edit" class="mr-28">등록</SLink>
      ...
      <template #data="{ data }">
        <table class="admin-table">
          ...
          <tbody>
            ...
            <tr v-for="(item, index) in data" :key="item.id" @click="$router.push(`/admin/coffee/manage/${item.id}`)">
              <td>
...
import SPageable from '~/components/admin/commons/SPageable.vue';
import SDropdown from '~/components/admin/commons/SDropdown.vue';
import SButton from '~/components/admin/commons/SButton.vue';
import SSearchBar from '~/components/admin/commons/SSearchBar.vue';
...
export default {
  ...
  async fetch() {
    ...
    this.tableData = await this.$axios.$get('/admin/coffees', {
      params: {
        ...this.queryOptions,
