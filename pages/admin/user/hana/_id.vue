...
export default {
  ...
  async asyncData({ params, $axios, redirect }) {
    const { id } = params;
    const detailData = await $axios.$get(`/admin/hana-bank/${id}`).catch(() => {
  ...
  async fetch() {
    ...
    this.tableData = await this.$axios.$get(`/admin/hana-bank/${this.id}/tickets`, { params: this.queryOptions });
