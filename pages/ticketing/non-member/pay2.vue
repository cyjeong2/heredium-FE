<template>
  <main class="container">
    ...
    <UDialogModal :is-show="$route.query.failed">
      <template #content>만 14세 미만의 어린이는 구매할 수 없어요.</template>
      <template #modal-btn2>
        <UButton
          w-size="100"
          class="feedback-btn"
...
export default {
  ...
  asyncData({ store, redirect }) {
    const payTargetData = store.getters['service/non-member-pay/getPayTargetData'];
    const isAgreeDone = store.getters['service/non-member-pay/getIsAgreeDone'];

    if (!payTargetData || !isAgreeDone) {
      redirect('/');
    }
  ...
  methods: {
    ...
    async check() {
      const jsonStoreData = JSON.stringify({
        isAgreeDone: this.isAgreeDone,
        payTargetData: this.payTargetData
      });
      localStorage.setItem('store', jsonStoreData);
      const returnUrl = `${window.location.protocol}//${window.location.host}/ticketing/non-member/pay3`;
      const errorUrl = `${window.location.protocol}//${window.location.host}/ticketing/non-member/pay1`;
